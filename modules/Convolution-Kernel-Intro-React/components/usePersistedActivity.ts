...
/**
 * Module-private SQLite adapter. Telemetry failure deliberately falls back to
 * usable in-memory state; it never introduces localStorage or locks the lesson.
 */
      ...
      if (result.entry) {
        try {
          restored = normalizeRef.current
            ? normalizeRef.current(result.entry.state)
