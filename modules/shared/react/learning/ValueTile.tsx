...
export function ValueTile({ label, value, tone, className, ...props }: ValueTileProps) {
  return (
    <div className={classNames('edu-value-tile', tone && `edu-value-tile--${tone}`, className)} {...props}>
      <Typography as="span" variant="bodySmall" tone="inherit" className="edu-value-label">{label}</Typography>
      <Typography as="output" variant="h2" tone="inherit" className="edu-value-number" data-i18n-ignore="true">{value}</Typography>
    </div>
  );
}
