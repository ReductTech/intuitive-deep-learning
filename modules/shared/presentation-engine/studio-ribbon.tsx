...
type RibbonTab = 'home' | 'insert' | 'design' | 'present';

export function StudioRibbon({ store, onInsert, onImport, onExport, onOpenJson }: {
  store: PresentationStore;
  onInsert: (type: 'title' | 'body' | 'image' | 'video' | 'model' | 'game') => void;
  onImport: () => void;
  onExport: () => void;
  onOpenJson: () => void;
}) {
  ...
  return <header className="pe-ribbon">
    ...
    <nav className="pe-ribbon__tabs">{([['home', '开始'], ['insert', '插入'], ['design', '设计'], ['present', '放映']] as const).map(([id, label]) => <button className={tab === id ? 'is-active' : ''} onClick={() => setTab(id)} key={id}>{label}</button>)}<span /><button className="pe-ribbon__advanced" onClick={onOpenJson}>高级</button></nav>
    <div className="pe-ribbon__commands">
      ...
      {tab === 'insert' ? <><RibbonGroup label="文字"><Command icon="T" label="标题" onClick={() => onInsert('title')} /><Command icon="¶" label="正文" onClick={() => onInsert('body')} /></RibbonGroup><RibbonGroup label="媒体"><Command icon="▧" label="图片" onClick={() => onInsert('image')} /><Command icon="▶" label="视频" onClick={() => onInsert('video')} /><Command icon="◇" label="3D" onClick={() => onInsert('model')} /></RibbonGroup><RibbonGroup label="互动"><Command icon="⌘" label="游戏/网页" onClick={() => onInsert('game')} /></RibbonGroup></> : null}
