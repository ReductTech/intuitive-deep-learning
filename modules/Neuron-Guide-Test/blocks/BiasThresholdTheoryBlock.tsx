...
export function BiasThresholdTheoryBlock({ onComplete }: { onComplete?: () => void }) {
  ...
  >
    ...
    <div className="ng-bias-theory__equivalence">
      <div className={step === 0 ? 'is-current' : ''}><Typography variant="bodySmall" tone="muted">门槛形式</Typography><Typography as="code" variant="bodySmall" tone="accent">wᵀx &gt; T</Typography></div>
      <Typography variant="h3" tone="warning">⇔</Typography>
      <div className={step === 1 ? 'is-current' : ''}><Typography variant="bodySmall" tone="muted">移项</Typography><Typography as="code" variant="bodySmall" tone="accent">wᵀx − T &gt; 0</Typography></div>
      <Typography variant="h3" tone="warning">⇔</Typography>
      <div className={step === 2 ? 'is-current' : ''}><Typography variant="bodySmall" tone="muted">偏置形式</Typography><Typography as="code" variant="bodySmall" tone="success">wᵀx + b &gt; 0</Typography></div>
    </div>
    {step === 2 ? <div className="ng-bias-theory__result-stack">
      <NoticeStrip className="ng-bias-theory__notice"><Typography as="strong" variant="bodySmall" tone="accent">偏置就是移入公式的判断门槛。</Typography><Typography as="span" variant="bodySmall" tone="muted">当门槛为 T 时，b = −T；这样所有判断都可以统一与 0 比较。</Typography></NoticeStrip>
      <div className="ng-bias-theory__current-result"><Typography variant="bodySmall" tone="warning">当前结果</Typography><Typography as="code" variant="bodySmall" tone="main">z = {formatScore(score)} + (−1.50) = {formatScore(centeredScore)}</Typography><Typography variant="bodySmall" tone={centeredScore >= 0 ? 'success' : 'accent'}>更倾向于“{tendency}”</Typography></div>
