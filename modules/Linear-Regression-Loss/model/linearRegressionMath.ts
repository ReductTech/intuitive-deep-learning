...
// A compact sample inspired by Galton's parent/child height study (in inches).
...
export function residual(point: HeightPair, slope: number, intercept: number) {
  return point.child - predict(point.parent, slope, intercept);
}

export function mse(points: HeightPair[], slope: number, intercept: number) {
  return points.reduce((sum, point) => sum + residual(point, slope, intercept) ** 2, 0) / points.length;
}

export function mae(points: HeightPair[], slope: number, intercept: number) {
  return points.reduce((sum, point) => sum + Math.abs(residual(point, slope, intercept)), 0) / points.length;
}
